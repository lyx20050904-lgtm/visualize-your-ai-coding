...
> 版本：2.2.0
> 狀態：P0+P1 全部實現，F16（注意力雷達）已實現；P2 F17（3D 活動地形圖）已規劃；新增 F18（Minimap 注意力模式）已實現
...
| F18 | **Minimap 注意力模式** | Minimap 縮略圖中的節點點位在編輯/讀取事件發生時，僅活躍節點保留顏色，其餘變灰（rgba(255,255,255,0.12)），讓用戶一眼定位 AI 當前操作的文件位置。無活躍事件時恢復全彩顯示 | 編輯/讀取事件狀態 | Minimap 節點顏色動態切換 |
...
### F18 Minimap 注意力模式 — 視覺規格

**動機：** 默認 minimap 所有節點點位全彩顯示，僅作為縮略導航使用，信息密度低。引入注意力模式後，minimap 成為「AI 當前焦點指示器」，一眼定位活躍文件。

**觸發邏輯：**
```
_更新 minimap 點位時:
  ├─ 檢查 editingIds.size > 0 或 readingIds.size > 0
  ├─ 若有活躍節點 → 進入注意力模式:
  │   ├─ 活躍節點: 保留顏色（編輯=#00D4FF，讀取=#7C8CFF，選中=#FF6B35）
  │   └─ 非活躍節點: rgba(255,255,255,0.12)
  └─ 若無活躍節點 → 正常全彩顯示
```
...
| 屬性 | 值 | 備註 |
|------|-----|------|
| 活躍編輯節點色 | `#00D4FF` | 與主圖譜編輯色一致 |
| 活躍讀取節點色 | `#7C8CFF` | 與主圖譜讀取色一致 |
| 選中節點色 | `#FF6B35` | 品牌色，高於編輯/讀取優先級 |
| 非活躍節點色 | `rgba(255,255,255,0.12)` | 接近透明，僅保留位置信息 |
| 點位半徑 | 1.8px | 不變 |
| 刷新機制 | `_refreshNodeColors()` 每次調用同步更新 minimap | 編輯/讀取狀態變更即時生效 |

**交互行為：**
- 無任何編輯/讀取活動時：minimap 全彩（同原行為）
- 單文件編輯/讀取時：僅該文件點位高亮
- 多文件並發編輯/讀取時：多個點位同時高亮
- 編輯與讀取疊加時：編輯優先級高於讀取
