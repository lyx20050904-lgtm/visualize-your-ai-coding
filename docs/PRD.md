...
> 版本：2.0.0
> 狀態：功能定義完成，待工程實現
> 本文檔替代 v1.0.0 所有章節，根據 2026-04-29 產品決策會議重新編寫
...
| F05 | **實時編輯動效** | 編輯中節點填充色變更 + feGaussianBlur 外光暈脈衝。消退使用 CSS transition 平滑過渡，非 setTimeout 跳變 | 文件 change 事件 | 節點外光暈（feGaussianBlur），防抖 500ms 後平滑消退 |
| F06 | **編輯熱力圖** | 節點外光暈強度與半徑反映累計編輯會話次數；每 30 秒重新計算一次發光強度，減少 DOM 操作 | 編輯會話計數（防抖 500ms 內連續 change 算一次會話） | 節點外光暈強度分級渲染 |
...
file-watcher._handleChange():
  ...
  └─ 每次 change → 重置 session 計時器（500ms）
      ↓
...
**計數規則：**
- 編輯會話定義：防抖窗口 500ms 內的連續 change 事件合併計為 1 次會話
...
| F13 節點詢問 Agent (single) | 按需查詢單個節點的角色說明與設計意圖 | 用戶點擊「詢問 Agent」按鈕 | 節點上下文組裝 → LLM 調用 → 結果渲染至細節面板 | 否（loading spinner） |
...
Request:
  {
    ...
    "contentSnippet": "export async function verifyToken..."  // 前 500 characters
  }
...
| **LLM 按需查詢失敗（F13）** | 同上 | 按鈕顯示錯誤提示「查詢失敗，請稍後重試」，不阻塞其他功能 |
